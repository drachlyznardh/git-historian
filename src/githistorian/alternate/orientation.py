...
def getOrientation(options):
	class Orientation:
		def __init__(self, highlight, flip):
			...
			self.SOURCE  = '{}', '{}'
			self.LCORNER = '22' if highlight else '┗┛┏┓'[flip] + ' ' # U+2517 251b 250f 2513
			self.RCORNER = '33' if highlight else '┛┗┓┏'[flip] + ' ' # U+251b 2517 2513 250f
			self.LMERGE  = '44' if highlight else '┫┣┫┣'[flip] + '┃' # U+252b 2523 252b 2523 2503
			self.RMERGE  = '55' if highlight else '┣┫┣┫'[flip] + '┃' # U+2523 252b 2523 252b 2503
			self.PIPE    = '66' if highlight else '┃┃┃┃'[flip] + '┃' # U+2503 2503 2503 2503 2503
			self.LARROW  = '77' if highlight else '«»«»'[flip] + ' '
			self.RARROW  = '88' if highlight else '»«»«'[flip] + ' '
			self.BROTHER = '99' if highlight else '┻┻┳┳'[flip] + '  ┃┃'[flip] # U+253b 253b 2533 2533 2503 2503
			self.HEAD    = 'aa' if highlight else '┳┳┻┻'[flip] + '┃┃  '[flip] # U+2533 2533 253b 253b 2503 2503
			self.NODE    = 'bb' if highlight else '•┃'                        # U+2022 2503
			self.ROOT    = 'cc' if highlight else '┻┻┳┳'[flip] + '  ┃┃'[flip] # U+253b 253b 2537 252f 2503 2503
